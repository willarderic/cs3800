...
class PeopleDB
{
  ...
  public:
    ...
    unsigned int addPerson(Person p)
    {
      ...
      cout << "Holding a write lock until addPerson() finishes" << endl;
    ...
    Person getPersonBySSN(string ssn)
    {
      ...
      cout << "Holding a read lock until getPersonBySSN() finishes" << endl;
    ...
    {
      Rlock lock(mtx); // Hold a read lock until this function exits
      cout << "Holding a read lock until getPersonByID() finishes" << endl;
      if(id >= SSNs.size())
      {
